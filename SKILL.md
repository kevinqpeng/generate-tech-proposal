...
1. 仅对**当前关联业务模块**进行完整技术设计（数据库、接口、流程、权限）。
2. 对关联业务模块（由其他团队并行开发）仅做**解耦设计**，仅描述协作边界与对接信息，不展开其内部实现。
3. 禁止在当前文档中详细设计其他模块的表结构、Service/Mapper 细节和完整业务流程。
4. 涉及跨模块协作时，统一输出“外部依赖协作信息”：接入方式、接口/事件名、关键入参、关键出参、责任团队、落地状态（待对接/已对接）。
5. 若存在待对齐项，使用“由外部团队负责实现，当前方案不展开”表述，不以假设实现替代真实协作边界。
...
7. **严格遵守模块边界** - 仅展开当前模块设计，其他模块仅保留协作边界与对接约定
...
- [ ] 跨模块协作均以“外部依赖协作说明”表达，并标注责任团队与状态
- [ ] 业务流程图清晰易懂
- [ ] 权限配置说明完整
